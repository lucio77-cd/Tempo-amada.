...
const Clima = (() => {
  ...
  async function buscarOpenMeteo() {
    ...
    // Extrai a previsão hora a hora do dia de amanhã (data = time[1]),
    // pegando alguns horários representativos para o gráfico
    const dataAmanha = time[1];
    const horasDesejadas = [0, 6, 9, 12, 15, 18, 21];
    const horaria = horasDesejadas.map((hora) => {
      const chaveHora = `${dataAmanha}T${String(hora).padStart(2, "0")}:00`;
      const indice = dados.hourly.time.indexOf(chaveHora);
      return {
        hora,
        temp: indice >= 0 ? Math.round(dados.hourly.temperature_2m[indice]) : null,
        categoria: indice >= 0 ? categoriaPorCodigoOpenMeteo(dados.hourly.weathercode[indice]) : "nublado",
      };
    }).filter((ponto) => ponto.temp !== null);
